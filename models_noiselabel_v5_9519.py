...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_decoder(self, x):
        ...
        if self.global_pool:
            x1 = x[:, :, :].mean(dim=1)
            # x2 = x[:, :, :].mean(dim=1)  # global pool without cls token
            # outcome = self.fc_norm(x)
        else:
